<script setup>
    ...
    const fields = [
        'country', 'region', 'city', 'street', 'building', 'flat', 'latitude', 'longitude', 'address_category_id'
    ];
    ...
    const action = async () => {
        ...
        try {
            ...
            const response = await updateAddress(data.value, addressId, authStore.token);
...
<template>
    ...
    <div class="mt-6 pb-6">
        <div class="grid grid-cols-12 gap-5">
            ...
            <div class="col-span-12 sm:col-span-8 xl:col-span-4">
                <div class="flex flex-col gap-6">
                    <InputGroup v-model="data.country" :error="data__errors.country[0]" label="Страна" placeholder="Россия"/>
                    <InputGroup v-model="data.region" :error="data__errors.region[0]" label="Регион" placeholder="Ростовская область"/>
                    <InputGroup v-model="data.city" :error="data__errors.city[0]" label="Город, населенный пункт" placeholder="Ростов-на-Дону"/>
                    <InputGroup v-model="data.street" :error="data__errors.street[0]" label="Улица" placeholder="Малиновского"/>
                    <InputGroup v-model="data.building" :error="data__errors.building[0]" label="Дом, строение" placeholder="125"/>
                    <InputGroup v-model="data.flat" :error="data__errors.flat[0]" label="Квартира" placeholder="125"/>
        ...
        <div class="grid grid-cols-12 gap-5">
            ...
            <div class="col-span-12 sm:col-span-8 xl:col-span-4">
                <div class="flex flex-col gap-6">
                    <InputGroup v-model="data.latitude" :error="data__errors.latitude[0]" label="Широта" placeholder="47.238704"/>
                    <InputGroup v-model="data.longitude" :error="data__errors.longitude[0]" label="Долгота" placeholder="39.614897"/>
                </div>
            </div>
        </div>
    </div>
