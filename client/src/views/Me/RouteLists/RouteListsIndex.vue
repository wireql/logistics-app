...
<template>
    <Modal size="small" position="middle" :show="showModal">
        ...
        <p class="mt-2 text-sm">
            Вы уверены, что хотите удалить данный маршрутный лист?
        </p>
        ...
        <div class="flex flex-col gap-3 overflow-x-auto">
            ...
            <table class="table-auto w-full">
                ...
                <tbody>
                    ...
                    <tr
                        v-else
                        v-for="route_list in response.data"
                        :key="route_list.id"
                    >
                        <td
                        ...
                        <td
                            class="text-xs font-normal py-2 border-t-1 border-gray-300 px-1 truncate max-w-56"
                        >
                            {{ route_list.description }}
                        </td>
                        <td
                            class="text-xs font-normal py-2 border-t-1 border-gray-300 px-1"
                        >
                            {{ route_list.delivery_date }}
                        </td>
                        <td
                            class="text-xs font-normal py-2 border-t-1 border-gray-300 px-1"
                        >
                            <div
                                class="flex justify-center font-bold p-1 rounded-full"
                                :class="getStatusClass(route_list.status.id)"
                            >
                                {{ route_list.status.name }}
                            </div>
                        </td>
                        <td
                            class="text-xs font-normal py-2 border-t-1 border-gray-300 px-1"
                        >
                            <div class="flex gap-2">
                                ...
                                <div
                                    v-on:click="deleteModal(route_list.id)"
