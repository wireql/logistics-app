...
onMounted(async () => {
    ...
    try {
        const [categoriesRes, bodyTypesRes, vehicleRes, vehicleStatusesRes] =
            await Promise.all([
...
<template>
    <div class="mt-6 flex flex-col sm:flex-row sm:items-center justify-between">
        <div>
            ...
            <div class="text-xs">Обновление информации об автомобиле</div>
        </div>
        <div class="flex items-center gap-[10px] mt-2">
            <router-link
                :to="{ name: 'VehiclesIndex' }"
    ...
    <BlockLoadingSpinner v-show="loading" />

    <div v-show="!loading" class="mt-6 pb-6">
        <div class="grid grid-cols-12 gap-5">
            ...
            <div class="col-span-12 sm:col-span-8 xl:col-span-4">
                <div class="flex flex-col gap-6">
                    <InputGroup
                        v-model="data.brand"
                        :error="data__errors.brand[0]"
                        label="Бренд"
                        placeholder="MAN"
                    />
                    <InputGroup
                        v-model="data.model"
                        :error="data__errors.model[0]"
                        label="Модель"
                        placeholder="TGX"
                    />
                    <InputGroup
                    ...
                    <div class="flex flex-col gap-[5px] w-full">
                        ...
                        <p
                            v-if="data__errors.vehicle_category_id !== null"
                            class="text-red-300 text-xs"
                        >
                            {{ data__errors.vehicle_category_id[0] }}
                        </p>
                    </div>
                    <div class="flex flex-col gap-[5px] w-full">
                        <label class="text-sm opacity-[60%]"
                            >Статус автомобиля</label
                        >
                        <select
                            v-model="data.vehicle_status_id"
                            class="text-sm w-full border border-gray-300 focus:border-gray-700 bg-dark-100 py-[6px] px-[9px] rounded-[6px]"
                        >
                            <option
                        ...
                        <p
                            v-if="data__errors.vehicle_status_id !== null"
                            class="text-red-300 text-xs"
                        >
                            {{ data__errors.vehicle_status_id[0] }}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="col-span-12 sm:col-span-8 xl:col-span-4">
                <div class="flex flex-col gap-6">
                    <div class="flex flex-col gap-[5px] w-full">
                        ...
                        <p
                            v-if="data__errors.body_type_id !== null"
                            class="text-red-300 text-xs"
                        >
                            {{ data__errors.body_type_id[0] }}
                        </p>
                    </div>
                    <InputGroup
